...
<div style="width: 80%; margin-left:10%; margin-right: auto;">
    ...
    <?php  
        ...
        $dataToday = $dataProviderMain->query->where(['TR.is_active' => 0, 'date_target' => date('Y-m-d')]);
        $dataProviderToday = new ActiveDataProvider([
            'query' => $dataToday,
        ]);
        $count_today = $dataProviderToday->getTotalCount();

        $dataTarget = $dataProviderMain->query
        ->where(['TR.is_active' => 0])
        ->andWhere(['not', ['TR.date_target' => null]]);
        $dataProviderTarget = new ActiveDataProvider([
            'query' => $dataTarget,
        ]);
        $count_target = $dataProviderTarget->getTotalCount();

        $dataPast = $dataProviderMain->query
        ->where(['TR.is_active' => 0])
        ->andFilterWhere(['<','TR.date_target',date('Y-m-d')]);
        $dataProviderPast = new ActiveDataProvider([
            'query' => $dataPast,
        ]);
        $count_past = $dataProviderPast->getTotalCount();
    ?>
    ...
    <?= Html::a('<span class="glyphicon glyphicon-flag"></span> Past Due'." <span class='label label-danger' style='font-weight:normal;'>".($count_past)."</span>",['/task/task/past-due'], Yii::$app->controller->action->id == "past-due" ? ['class' => 'btn btn-lg btn-primary'] : ['class' => 'btn btn-lg btn-default']);  ?>

    <?= Html::a('<span class="glyphicon glyphicon-screenshot"></span> Target Task'." <span class='label label-danger' style='font-weight:normal;'>".($count_target)."</span>",['/task/task/target-task'], Yii::$app->controller->action->id == "target-task" ? ['class' => 'btn btn-lg btn-primary'] : ['class' => 'btn btn-lg btn-default']);  ?>
